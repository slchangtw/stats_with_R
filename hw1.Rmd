...
As the results indicates below, a company with profits above 9.87 billion USD can be in the top two percent.
...
The top three contries are "South Korea", "India" and "Germany", and the following table lists.
...
The median of profits in Germany (3.45) exceeds the median profit for all companies (2.95), while the medians of profits in South Korea and India do not.
...
The median profit of sampled data (`r format(median(sampled_forbes2016$Profits, na.rm = TRUE), digits = 3)`) is smaller than median market value of all companies (`r format(median(sub_forbes2016$Market.Value, na.rm = TRUE), digits = 3)`).

The mean profit of sampled data (`r format(mean(sampled_forbes2016$Profits, na.rm = TRUE), digits = 3)`) is also smaller than mean market value of all companies (`r format(mean(sub_forbes2016$Market.Value, na.rm = TRUE), digits = 3)`).
...
As the plots display, there exists no extreme skewness but slight left skewness. That is, the outliers appear more often in the left side of the distribution. The boxplot also shows that 4 companies has profits less than lower limit (Q1 - 1.5IQR). 
