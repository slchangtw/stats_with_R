...
```
The observed standard deviation is 5
...
```
The box plot is not symmetric which implies that there values in the second quartile are less desnsly packed than the 3rd quartile. Also there are many more outliers about 4.3 than below 3.2
...
```
The second quartile is more distributed than the second quartile, there are more observation below 5.5 than above 6.1. There are two outliers that are close to 7.0 value
...
```
We observe that the max income for male is 28.000 while that for a female is 21. ALso the minimum value for male is 0.00 while that of female is 0.125. The mean income for female home owners is higher that mean income for male home owners. 
```{r}
ggplot(creditcard) +
...
```
As observed from the above representation we see that there are about 6 outliers for male home owners while there arent any outliers for female homeowners. Also the spread of income is more even in females when compared to males. Observation for males are larger in number than for females.The distribution is positive skewed. Interquartile range is smaller in males as shown in the box plot but the standard deviation is larger in males due to outliers.
...
p_larger
```
the probability of a t-distributed random variable being larger than or equal the above calculated t-statistic score of 0.340173 using a t-distribution with n1 + n2 - 2 degrees of freedom is 0.366985.
...
```
The null hypothesis to be tested is if the difference between mean of income between male and female is equal to 0
Alternatiev hypothesis is the difference in mean is not equal to 0
Since the P value is greater than the 0.05 we cannot reject the null hypothesis in favour of the alternative hypothesis
we also see that the t-statistics value is similar to what we had calculated individually.
